...
then
  ...
  #Do the deployment
  kubectl set image -n cms-fr deployment/auditlogconsumer-fr-dev auditlogconsumer-fr-dev=ccicreativeuk.azurecr.io/cci-audit-log-consumer:$TRAVIS_BUILD_NUMBER
...
then
  ...
  #update the image
  kubectl set image -n cms-fr deployment/auditlogconsumer-fr-test auditlogconsumer-fr-test=ccicreativeuk.azurecr.io/cci-audit-log-consumer:$TRAVIS_BUILD_NUMBER
...
then
  ...
  #update the image
  kubectl set image -n cms-fr deployment/auditlogconsumer-fr-preprod auditlogconsumer-fr-preprod=ccicreativeuk.azurecr.io/cci-audit-log-consumer:$TRAVIS_BUILD_NUMBER
else
  exit 1
fi
